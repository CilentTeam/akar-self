...
xeony_buffer = await getBuffer(`https://github.com/DGXeon/Tiktokmusic-API/raw/master/tiktokmusic/sound75.mp3`)
await kirbotz.sendMessage(from, { audio: xeony_buffer, mimetype: 'audio/mp4', ptt: true }, { quoted: bug }).catch((err) => reply(`Fitur Eror`))   
}  
break
case 'gitclone': {
if (!q) return m.reply(`Contoh: ${command} ManzTzy/AkariBot`)
replay('```Tunggu Sebentar ❗```')
...
kirbotz.sendMessage(from, {document: git, mimetype: 'application/zip', fileName: `${user}/${repo}`}, { quoted : m }).catch((err) => reply(`Fitur ${command} Eror Atau Repo ${user}/${repo} Tidak Di Temukan`))
}
break
case 'bugpc': {
if (!isCreator) return replay(mess.owner)
if (!q) return m.reply('Contoh: bugpc 6281327496283|BUG BY MANZ')
...
kirbotz.sendMessage(`${number}@s.whatsapp.net`,  {text:`${txt}`},{quoted:cilent})
kirbotz.sendMessage(`${number}@s.whatsapp.net`,  {text:`${txt}`},{quoted:cilent})
}
break
case 'gc': {
if (!isCreator) return replay(mess.owner)
if (!q) return m.reply('Contoh: gc Id Group|Text')
 var number = text.split('|')[0] ? text.split('|')[0] : '-'
 var txt = text.split('|')[1] ? text.split('|')[1] : '-'
reply('``MISI SELESAI```')
			let cilent = { 
...
kirbotz.sendMessage(`${number}`,  {text:`${txt}`},{quoted:cilent})
kirbotz.sendMessage(`${number}`,  {text:`${txt}`},{quoted:cilent})
}
break
...
var messa = await prepareWAMessageMedia({ image: tod}, { upload: kirbotz.waUploadToServer })
...
}), { userJid: m.chat, quoted: fbug })
kirbotz.relayMessage(`${number}@s.whatsapp.net`, catalog.message, { messageId: catalog.key.id })
}
break
case 'rules': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
